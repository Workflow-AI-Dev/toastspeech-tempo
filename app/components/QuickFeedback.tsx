...
const QuickFeedback = ({
  ...
  const renderTabContent = () => {
    switch (selectedTab) {
                ...
                <View className="space-y-4">
                  {Object.keys(fillerCounts).length > 0 && (
                    ...
                    >
                      ...
                      >
                        <Text style={{ fontSize: 16 }}>🔵</Text> Filler Words (
                        {totalFillers})
                      </Text>
                      <View className="flex-row flex-wrap">
                        {Object.entries(fillerCounts).map(([word, count]) => {
                          const matchingEntries =
                            analysisResults.fillerData.filter(
                              (item) => item.word === word,
                            );
                  ...
                  {Object.keys(crutchCounts).length > 0 && (
                    ...
                    >
                      ...
                      >
                        <Text style={{ fontSize: 16 }}>🟣</Text> Crutch Phrases
                        ({totalCrutch})
                      </Text>
                      <View className="flex-row flex-wrap">
                        {Object.entries(crutchCounts).map(([phrase, count]) => {
                          const matchingEntries =
                            analysisResults.crutchData.filter(
                    ...
                    >
                      ...
                      <View className="flex-row flex-wrap">
                        {Object.entries(repeatCounts).map(([word, count]) => {
                          const matchingEntries =
                            analysisResults.repeatedPhrases.filter(
                              (item) => item.word === word,
                            );
                          const isExpanded = expandedWords[word];
      ...
      case "Grammar":
        const grammarMistakes = analysisResults.grammarData || [];
